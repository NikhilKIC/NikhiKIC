...

{
  ...
  $id=$_POST['book_id'];  
//this is defining that book_id is the variable $id

  if($id!="")
  {  
      $sql="DELETE FROM `book` WHERE `book`.`b_id` ="."'$id'";
      //when the ID of the book that the admin wants deleted is entered, it searches it through the database and then procedes to delete it
	$data = mysqli_query($conn, $sql);
	
      if($data)
	  {
	    $msg= "Book has been Delete Successfully!"; //if the programme was correctly able to identify the PDF that needs to be deleted and deletes it, it will display the message "Book has been deleted sucessfully!"
	  }
	  else
	  {
	    $msg= "Something Went Wrong... please try again"; //if the programme was unable to find the book in the database and delete it, it will display the error message "Something went wrong, please try again"
	  }
}
    else
	  { 
	   $msg="Book ID Required";  //if the book ID field was left blank and empty, "book ID required will appear on the screen to alert the reader that they need to fill in the feild.
	  }
}
?>
<html>
