...
//this is defining the different variables
  ...
  {  
	 ...
	 $accepted=array("pdf");
//this this sorts the file in alphabetical order and adds the name, temp name and the mini url for the ebook. This will only work if the publication is a PDF doc. 
	 
	 
	if(!in_array(pathinfo($file_name,PATHINFO_EXTENSION),$accepted))
	{
	$msg= $file_name."<br> is not an acceptable file type";
	} //if a specific file type is not acceptable, it will display the error message "... this is not an acceptable file type."
  //in this it only accepts the file extention .pdf which is a PDF file. Any other file extension will be declined.
	else
	{
      ...
      $insert="INSERT INTO `book`(`b_id`,`booksname`,`authorname`,`file_name`,`path`) VALUES('".$id."','".$bookname."','".$authorname."','".$new_file_name."','".$fpath."')";
      $data=mysqli_query($conn,$insert); 
      if($data)
	  {
	  //this is the code to add a pfd to the phpmyadmin database, it takes the the pdf name, ID and author's name and adds it into the system.
    //it adds the book ID (b_id), books name, authors name, file name and the path for it into each of the rows in the phpmyadmin database
	  
	    $msg= "Sccessfully Added";
	  } //once it is sucessfully added, it will present the message "Successfully added"
      else{
          $msg="Something Went Wrong";
      } //if there has been an error during the uploading, it will display the message "Something went wrong"
   }
	  else
	  {
	    $msg= "All field are required";
	  } //if there are any feilds that are present are left blank, it will display the error message "all feilds are required"
